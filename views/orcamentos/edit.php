...
if (!function_exists('obterTextoPadraoSistema')) {
    function obterTextoPadraoSistema(PDO $db, string $chave, string $fallback = ''): string
    {
        try {
            ...
            error_log('[orcamentos/edit.php] Falha ao obter texto padrão ' . $chave . ': ' . $e->getMessage());
            return $fallback;
        }
    }
}
...
// No edit, o orçamento mantém o texto salvo nele; o botão permite substituir pelo padrão atual da matriz.
...
$textoPadraoObservacoesMatriz = obterTextoPadraoSistema($db, 'observacoes_gerais_padrao', $fallbackObservacoesOrcamento);
$textoPadraoCondicoesMatriz = obterTextoPadraoSistema($db, 'condicoes_pagamento_padrao', $fallbackCondicoesOrcamento);

$textoObservacoesOrcamento = !empty($orcamentoDados['observacoes']) ? $orcamentoDados['observacoes'] : $textoPadraoObservacoesMatriz;
$textoCondicoesOrcamento = !empty($orcamentoDados['condicoes_pagamento']) ? $orcamentoDados['condicoes_pagamento'] : $textoPadraoCondicoesMatriz;
...
// --- 3.1 BLOCO AJAX PARA SALVAR CLIENTE PELO MODAL ---
if (isset($_POST['ajax']) && $_POST['ajax'] === 'salvar_cliente_modal') {
    header('Content-Type: application/json; charset=utf-8');

    try {
        $clienteModel->nome = trim($_POST['nome'] ?? '');
        $clienteModel->telefone = trim($_POST['telefone'] ?? '') ?: null;
        $emailPost = trim($_POST['email'] ?? '');
        $clienteModel->email = $emailPost !== '' ? filter_var($emailPost, FILTER_VALIDATE_EMAIL) : null;
        $clienteModel->cpf_cnpj = trim($_POST['cpf_cnpj'] ?? '') ?: null;
        $clienteModel->endereco = trim($_POST['endereco'] ?? '') ?: null;
        $clienteModel->cidade = trim($_POST['cidade'] ?? '') ?: null;
        $clienteModel->observacoes = trim($_POST['observacoes'] ?? '') ?: null;
        ...
        if ($emailPost !== '' && $clienteModel->email === false) {
            http_response_code(422);
        ...
        echo json_encode(['success' => true, 'cliente' => $clienteCriado]);
        exit;

    } catch (Exception $e) {
        http_response_code(500);
        error_log('Erro AJAX salvar_cliente_modal (edit orçamento): ' . $e->getMessage());
                                ...
                                <tbody>
                                    <?php if (empty($itensOrcamento)): ?>
                                        <tr class="no-items-row">
                                            <td colspan="7" class="text-center text-muted">Nenhum item adicionado a este orçamento ainda.</td>
                                        </tr>
                                    <?php else: ?>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="observacoes_gerais" name="observacoes_gerais" rows="6" placeholder="Ex: Cliente solicitou montagem especial..."><?= htmlspecialchars($textoObservacoesOrcamento ?? '') ?></textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="condicoes_pagamento" name="condicoes_pagamento" rows="5" placeholder="Ex: Entrada, saldo, PIX, locação estendida..."><?= htmlspecialchars($textoCondicoesOrcamento ?? '') ?></textarea>
                                    <small class="form-text text-muted">Este texto será salvo somente neste orçamento. Para alterar o padrão definitivo, use Configurações &gt; Textos Padrão.</small>
                                </div>
                                <div class="form-group">
...
<style>
    ...
    .orcamento-busca-barra .col-md-7 { position: relative; }
    .orcamento-busca-barra #sugestoes_produtos { z-index: 1060 !important; }
    ...
    #tabela_itens_orcamento td { vertical-align: middle !important; }
...
$('#data_devolucao_prevista').on('change dp.change', function() {
    ...
    if (!validarSequenciaDatasLogisticaCreate(false)) {
        marcarLinhasComoPeriodoInvalido();
    }
    calcularTotaisOrcamento();
...
window.TEXTOS_PADRAO_ORCAMENTO = " . json_encode([
    'observacoes' => $textoPadraoObservacoesMatriz,
    'condicoes' => $textoPadraoCondicoesMatriz,
