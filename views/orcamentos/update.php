...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id']) && is_numeric($_POST['id'])) {
    ...
    // Preencher os dados atualizados do orçamento a partir do formulário
    ...
    $orcamentoModel->status = $_POST['status'];
    ...
    if ($orcamentoModel->update()) {
        ...
        if (isset($_POST['itens']) && is_array($_POST['itens'])) {
            // Primeiro, remover itens existentes para evitar duplicação
            $conn->prepare("DELETE FROM itens_orcamento WHERE orcamento_id = :orcamento_id")
                ->execute(['orcamento_id' => $id]);
            
            // Agora, adicionar os itens atualizados
            $itens = [];
            ...
            $orcamentoModel->salvarItens($id, $itens);
        }
        $_SESSION['success_message'] = "Sucesso: Orçamento #$orcamentoModel->numero atualizado com sucesso!";
        header('Location: show.php?id=' . $id);
        exit;
    } else {
        $_SESSION['error_message'] = "Erro: Falha ao atualizar o orçamento #$orcamentoModel->numero. Tente novamente.";
        header('Location: edit.php?id=' . $id);
        exit;
    }
} else {
    $_SESSION['error_message'] = "Erro: Método de requisição inválido ou ID não fornecido.";
    header('Location: index.php');
    exit;
}
?>